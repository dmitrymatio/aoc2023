...
async function processLineByLine() {
    ...
    for await (const line of rl) {
        ...
        */
        ...
        const minCubes: { [key: string]: number } = { red: 0, green: 0, blue: 0 };
        ...
        for (const game of gameData) {
            for (const entry of game) {
                ...
                if (value > minCubes[color as keyof typeof minCubes]) {
                    minCubes[color as keyof typeof minCubes] = value;
                }
            }
        }

        const minCubesPower = minCubes["red"] * minCubes["green"] * minCubes["blue"];
        sum += minCubesPower;
    }
